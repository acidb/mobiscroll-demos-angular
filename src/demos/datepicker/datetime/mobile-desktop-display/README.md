...
- Use `controls: ['date']` (wheel-style, not the month-grid `calendar` control) with `display` set to `'inline'` (embedded, no popup), `'anchored'` (opens next to the input), `'top'` (modal entering from the top), `'bottom'` (modal entering from the bottom), or `'center'` (centered modal) — five separate `Datepicker` instances, one per mode.
